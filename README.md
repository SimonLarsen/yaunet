...
Just import the `UNet` class from `yaunet.unet` and instantiate:
...
A number of blocks are implemented in `yaunet.blocks`.
Pass the blocks' constructors to `UNet` to use:
...
Use `functools.partial` to set block parameters:
...
All blocks support AdaLN-Zero-like conditioning \[1\].
...
* \[1\] Peebles, William, and Saining Xie. "Scalable diffusion models with transformers." *2023 IEEE/CVF International Conference on Computer Vision (ICCV)*. IEEE, 2023.
